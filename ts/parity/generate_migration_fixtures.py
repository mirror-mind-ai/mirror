...
# Fixtures the forward cascade cannot produce, generated by a dedicated path (see
# _write_016_fixture). Declared here as the single source of truth so the
# drift-guard test (test_migration_fixtures_snapshot.py) covers them alongside the
# cascade stems — both the on-disk coverage check and the regenerate-and-compare.
HAND_AUTHORED_CHECKPOINTS: tuple[str, ...] = ("016",)
