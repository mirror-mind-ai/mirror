...
// close hooks. (Until CV22.DS10.TS5 a reverted tail reported `{ handled:
// false }` so the caller could fall back to Python; the revert left with the
// fallback, and every result is now an answer.) `repair-journeys --apply`
// needs the dated zip backup from the runtime (CV22.DS7.TS1) and refuses, like
// Python, when it is not available.
...
import type { LoggerRuntime } from "#conversation/loggerRuntime.ts";
import {
  sessionMaintenance,
  sessionStart,
  sessionStartFast,
...
export interface LoggerCliResult {
  stdout: string[];
  stderr: string[];
  exitCode: number;
}
...
export async function runConversationLoggerCommand(
  ...
  if (mirrorHome.error) {
    return { stdout: [], stderr: [mirrorHome.error], exitCode: 1 };
  }
  const sessionOption = extractOption(args, "--session-id");
  if (sessionOption.error) {
    return { stdout: [], stderr: [sessionOption.error], exitCode: 1 };
  }
  if (args.length === 0) {
  ...
  switch (command) {
    ...
    case "switch": {
      ...
      return ok([
        conversationId ? `New conversation created: ${conversationId}` : "No active session found.",
      ]);
    }
    ...
    case "repair-journeys": {
      ...
      if (limitOption.error) {
        return { stdout: [], stderr: [limitOption.error], exitCode: 1 };
      }
      ...
      if (limitOption.value !== null) {
        ...
        if (!/^\s*[+-]?\d+\s*$/.test(limitOption.value)) {
          return {
            stdout: [],
            stderr: [`ValueError: invalid literal for int() with base 10: '${limitOption.value}'`],
            exitCode: 1,
          };
        }
