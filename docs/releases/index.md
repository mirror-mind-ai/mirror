...
- [v0.31.5 — One Code, One Package](v0.31.5.md) — maintenance patch that closes the roadmap-resolution class v0.31.4 only partially fixed. One shared resolver (`memory.builder.story_paths`) now backs both Ariad Expand and the CLI's Plan/checkpoint lookup, superseding v0.31.4's narrower, same-day `_resolve_existing_item_directory` fix (discovered independently on `kia-desktop`) while this release's own investigation traced a second occurrence of the CR048 defect (CV22.DS7). Two packages sharing a code, or a resolved package whose candidate table can't be parsed, now render a first-class `EXPAND_BLOCKED` refusal instead of resolving silently or fabricating a generic story; a new CI check makes "one code, one package" a build-time guarantee. Closes D-012 (candidate-table code-cell path sanitization) as a direct consequence of touching the functions its revisit trigger named. Reconciled with v0.31.4 by rebase, keeping its regression test as additional coverage.
- [v0.31.4 — Roadmap-Aligned Story Expansion](v0.31.4.md) — maintenance patch that stops Ariad's Builder from materializing a phantom, parallel story-package tree when expanding a Delivery Story. `_artifact_directory` derived roadmap folders from code plus title, but hand-authored roadmaps use CV/DS slugs (`cv3-build-de-producao`) no title derivation can reproduce, so expanding CV3.DS22 wrote a boilerplate `cv3-manage-100-funcional/cv3-ds22/` tree instead of the real `cv3-build-de-producao/cv3-ds22-manage-100-funcional/` node. Expand now resolves the existing folder by matching each `index.md`'s `# CODE — Title` heading to the item code (the same scan the pull inspector uses) and only falls back to title derivation for genuinely new items. Pinned by a nested-folder regression test and validated against the live kia-desktop roadmap.
- [v0.31.3 — Ariad DS-Plan Artifact Integrity](v0.31.3.md) — maintenance patch that makes the Ariad Delivery Story plan lifecycle's artifacts trustworthy. Fixes phantom `ARTIFACTS_MATERIALIZED` rows for a never-written `test-guide.md` and a falsely "updated" `index.md` (AF-002, AF-006) by driving the surface from the writer's real manifest and materializing the full package without clobbering authored content. Scaffolds every `plan_contract` section in `plan.md` (AF-003), then — after a six-lens technical review chose pure-preserve over a surgical status patcher — removes mutable status from the document so `approve-delivery-story-plan` preserves the authored plan instead of regenerating it, and flags any section left unfilled. Records the warn-not-block choice and the D-012 code-segment sanitization gap as conscious exclusions.
- [v0.31.2 — Filesystem-Safe Slug Consolidation](v0.31.2.md) — maintenance patch that fixes an `OSError [Errno 63]` (File name too long) crash in `build pull-item` when a Delivery Story candidate title is paragraph-length, by consolidating four drifted kebab `_slugify` copies into one accent-stripping, length-capped `memory.utils.kebab_slug`. Closes the same latent `mkdir` crash in the Explorer handoff writer, makes the roadmap package writer and locator derive the same path by construction, and adds an empty-slug guard. Reviewed through five personas before implementation; records D-012 (code-cell path-separator sanitization) and D-013 (the `transcript_export` slug sibling) as conscious debt.
- [v0.31.1 — Change Request Terminal Verbs](v0.31.1.md) — patch release that adds `change-request park/reject/promote` (and `refinement-story park`) to the Ariad Refinement Workbench, closing the gap that forced an off-contract production storage write for CR023. Planned through a six-persona review before implementation; fixes a latent `_flow_event` bug, a surface-marker injection path, and a progress-surface trust bug found during that review. CR023 is Navigator-confirmed reconciled; an open roadmap-parser investigation is closed in documentation; TD-002 is registered.
- [v0.31.0 — Stabilization & Robustness](v0.31.0.md) — minor release that delivers the bulk of the CV9.E2 Stabilization & Robustness epic: extraction failure isolation/quarantine and idempotency, lexical-only search degradation, explicit call timeouts, content-mediated injection fencing across all five model surfaces, an identity-write allowlist, metadata-default cost observability, and an `eval --all` release gate — plus the AI Engineering Audit P0 tier and most P1/P2 findings. CV9.E2.S2 is deferred; the model-behavior gate is satisfied by a recorded D-005 waiver.
