[< Parent](../index.md)

# CV23.DS7.US1 — Release and Consumer Return

**Status:** ✅ Done
**Type:** User Story

---

## User Story

As a user,
I want to Release and Consumer Return,
So that I can receive the value of this story.

## Outcome

Navigator can validate Release and Consumer Return as an observable behavior.

## Acceptance Behavior

```text
Given the user is ready for Release and Consumer Return
When the user performs the planned action
Then the expected observable behavior is visible
And unrelated Delivery Story scope remains untouched
```

## Scope

- Release and Consumer Return

## Out Of Scope

- Sibling Delivery Story scope.

## Validation

Navigator-visible validation route plus automated checks.
