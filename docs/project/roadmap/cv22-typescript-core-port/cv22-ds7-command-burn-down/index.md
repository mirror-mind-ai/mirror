...
| [CV22.DS7.US5](cv22-ds7-us5-extraction-lifecycle/index.md) | Extraction lifecycle: deterministic core | User Story | Deterministic `conversation-logger` core, hook entries, the `conversations append` boundary, and the budgeted extraction driver answered by TS; seven subcommands flipped (high risk — writes) | 🔵 Implementation complete — pending validation |
| [CV22.DS7.US10](cv22-ds7-us10-extraction-lifecycle-session-composites/index.md) | Extraction lifecycle: session composites & LLM-tail flips | User Story | Session composites, diagnose/repair, backfills, and the flips that cross `end_conversation`'s LLM close tail, behind the replay transport (high risk — writes + provider seam) | 🟡 Planned |
