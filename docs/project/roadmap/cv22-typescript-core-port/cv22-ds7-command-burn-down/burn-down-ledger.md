...
| Extension context runtime | (`ext`/`extensions` context path) | — | DS7.TS2 | ✅ done — **compat host deleted 2026-09-21 (DS10.TS2)**; a provider with no `provider_runtime` is skipped with a `no_provider_runtime` diagnostic and `mirror load` continues |
...
| ~~`ext list`, `ext <id>`, `ext <id> bind\|unbind\|bindings\|migrate`~~ | DS7.TS4 | ✅ **Answered by TS since 2026-09-16.** 40 recorded dispatch cases and 24 binding/migration cases, plus the `ext_bindings` write probe. **The 40-case dispatch corpus retired 2026-09-21 (DS10.TS2)** when the host it graded against was deleted: 12 survive as direct assertions, 10 migrated to the declared path, 18 were retired with a recorded disposition (CPython coercion/traceback semantics and the live-registry listing, which nothing reproduces) |
| ~~`ext <id> <extension-subcommand>`~~ | DS7.TS4 | ✅ **Answered by TS since 2026-09-16.** The plan input was decided by the Navigator (D1, 2026-09-16): **both halves** — a declared `mirror-cli-v1` runtime on the manifest's existing `cli.subcommands[]` is executed directly, and anything else fell per-SUBCOMMAND to a `cli` mode of TS2's compat host. **That fallback is gone since 2026-09-21 (DS10.TS2):** the host is deleted, an undeclared subcommand refuses with one line naming the fix, and the `ext <id>` listing renders from the manifest instead of the live registry. Validated against all seven installed extensions on a copy of the real home, and one real read-only subcommand on the real home |
