...
| Leaf | Owner | Note |
|------|-------|------|
| `build` — 27 in-scope leaves (29 subcommands, **47** leaves in Python) | DS7.US8 | Self-hosting story; see the [US8 package](cv22-ds7-us8-builder-ariad-tree/index.md). Leaf count corrected 2026-09-13 at plateau 1 by enumerating the subparsers (was 42). **In progress, nothing routed:** plateaus 1–2 have ported 6 of the 27 leaves behind no route at all — `inspect-method`, `pull-candidates`, `adopt`, `prepare-templates`, `sync-cursor`, `check-implementation` — plus the cursor, the method DSL, and every activation surface. `routing.ts` still sends **all** of `build` to Python; the gate arrives at plateau 8 and the flip at plateau 9 |
