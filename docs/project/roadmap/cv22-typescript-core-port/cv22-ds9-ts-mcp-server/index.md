...
**Status:** ✅ **Done — 2026-09-19 (4/4).** US1 and US2 2026-09-17, TS2 2026-09-18, TS1 2026-09-19. `python -m memory mcp` is answered by TypeScript, launched by the plugin manifest, and bounded
**Type:** Delivery Story
...
| CV22.DS9.TS1 | Wallet and abuse guards | Technical Story | A shared sliding-window rate guard counted from the `llm_calls` ledger and attributed to this surface, argument bounds with per-tool `limit` caps, and refusals that are agent-readable `isError` results | ✅ **Done — 2026-09-19.** D2 answered **constant `mcp` marker with one global rate**, deviating from the rider's "per-tool" wording because both paid tools cost one embedding per call; D9 declined to cap `journey_status` and captured its shape as CR087. The E2E ran twice: the first session found the refusal text reading as *"a way around the limit"* and an agent continuing four calls past "do not retry"; the corrected wording stopped it at the first refusal |
