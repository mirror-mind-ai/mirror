...
| [CV22.DS9](cv22-ds9-ts-mcp-server/index.md) | TS MCP Server | Threat model first (RS005: transport/trust boundary, per-tool permission scoping; AI-19: per-tool rate/budget guards against denial-of-wallet), then port `python -m memory mcp` to TS — stdio JSON-RPC protocol core, the seven read/context tools, the wallet guards, and the plugin manifest flip | 🟢 In Progress (**1/4**) — pulled and authored 2026-09-17 from code: 363 Python lines, stdio (no listener), seven read-only tools, two commits of lifetime churn, and every underlying capability already TS-owned. **US1 and US2 done 2026-09-17** — dispatch, registry, stdio loop, and all seven tools byte-identical to the oracle (12/12 on a copy of the 50 MB production database), threat model written and reviewed. Order corrected by US2 D12: **TS2 before TS1**, because the read-only server records no spend for TS1's wallet guard to count |
