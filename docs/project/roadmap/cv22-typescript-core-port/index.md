...
| [CV22.DS9](cv22-ds9-ts-mcp-server/index.md) | TS MCP Server | Threat model first (RS005: transport/trust boundary, per-tool permission scoping; AI-19: per-tool rate/budget guards against denial-of-wallet), then port `python -m memory mcp` to TS — stdio JSON-RPC protocol core, the seven read/context tools, the wallet guards, and the plugin manifest flip | 🟢 In Progress — pulled and authored 2026-09-17 from code: 363 Python lines, stdio (no listener), seven read-only tools, two commits of lifetime churn, and every underlying capability already TS-owned. Four candidate stories: US1 protocol core, US2 tools, TS1 wallet/abuse guards, TS2 manifest cutover |
