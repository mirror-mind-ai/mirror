...
| [CV22.DS2.TS3](cv22-ds2-ts3-reusable-real-db-copy-parity-harness/index.md) | Reusable Real-DB-Copy Parity Harness | Technical Story | Promote the US1 real-DB-copy validation route into a safe, documented, reusable harness before handing off the rest of DS2 | ✅ Done |
| [CV22.DS2.US2](cv22-ds2-us2-detect-persona-parity/index.md) | `detect-persona` Parity | User Story | TS `detect-persona` reproduces the Python routing score/threshold decision on the golden corpus | ✅ Done |
| [CV22.DS2.US3](cv22-ds2-us3-journeys-memory-listing-parity/index.md) | Journeys & Memory Listing Parity | User Story | TS read of journeys and memory listing reproduces Python's ordered/behavioral output on the golden corpus | ✅ Done |

**TS1**, **TS2**, **US1**, and **TS3** are done — the `node:sqlite` seam, parity-critical decoders (`blobToFloat32`/`parseUtcMs`), synthetic golden-corpus generator, ordered-id grader, durable TS hybrid ranker, and reusable redacted real-DB-copy parity harness are in place. US1 extended the golden schema with the remaining ranker inputs (`use_count`, `relevance_score`, `access_count`, `last_accessed_at`, and lexical/text surface), proved synthetic parity in CI-level tests, and passed real-DB-copy parity across five probes. TS3 made the real-DB-copy route portable through a repository-generated synthetic demo DB and redacted evidence by default. **US2** (first Baton 2 slice) is done — the pure `detect-persona` router is ported into a durable TS module with exact behavioral parity on a branch-covering synthetic golden, and the reusable harness now carries a `detect-persona` probe path. **US3** (journeys & memory listing) is done — journey listing ported as a pure TS module with a committed golden, memory listing ported as a seam SQL read model (query builder + mapper) with ordering proven against a copied DB in the reusable harness (option B). With US3 closed, **DS2 is complete**: the read-only deterministic command set (`search`, `detect-persona`, journeys, memory listing) reproduces Python over the shared seam, validated on synthetic goldens in CI and on real-DB copies through the redacted harness. This is the Baton 2 plateau — the foundation is ready to be put behind the Pi TS front door (CV22.DS3).
