...
| 2 | The program's name | `PROGRAM` in `ts/src/util/program.ts` is the name every usage line and hint prints. Rename it with the `bin`; the goldens `ts/test/goldens/README.md` lists take the same edit again: the seven of D12, and `builder-roadmap` and `builder-orientation`, which carry the program in the Pull command since [CR002](../../../../refinement/rs001-ariad-runtime-trust/cr002-cursor-sync-roadmap-selection.md) | TS5 D12, CR002 |
