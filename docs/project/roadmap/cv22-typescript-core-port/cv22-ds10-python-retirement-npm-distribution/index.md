...
| [CV22.DS10.TS5](cv22-ds10-ts5-python-core-deletion/index.md) | Python core deletion | Technical Story | `src/memory/`, its tests, the `uv` and `pyproject` Python surface, the front door's `fallbackPython` path, and the `MIRROR_TS_*` revert gates removed; existing `memory.db` files keep working; every runtime operates over TS only. Separately Navigator-authorized | 🟢 **In Progress — pulled and planned 2026-09-23, panel-reviewed, plateaus 0 and 1 of 5 done.** The oracle is frozen and a 29-family capture taken against a real database copy; all three guards are Node with tests, each agreeing byte-for-byte with its Python original; identity and version left `pyproject.toml` before it is deleted (D1); **D-025 paid** — TypeScript is the sole migration custodian and a declined migration now fails an update instead of passing it; and every runtime hook is Node, proven 10/10 against the Python hooks by row-diff. **Nothing is deleted yet.** Four inventory findings are recorded in the story's [inventory.md](cv22-ds10-ts5-python-core-deletion/inventory.md), the sharpest being that US2's Skill Invocation Gate could not see thirteen hook files because its pattern was `uv run python -m memory` and they said `python3 -m memory` |
