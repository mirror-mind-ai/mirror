...
Implemented the Soul Mode Integration Proposal. `memory soul review` now renders a `☾ INTEGRATION PROPOSAL` card with optional proposal-only sections for journal, self, shadow, ego behavior, persona, and leave open. The renderer rejects empty reviews, omits empty categories, preserves the `proposal only — nothing changed` footer, and intentionally excludes journey identity because that category is not mature enough for this release. The Pi Soul Mode skill now uses the post-closing invitation `Há material vivo que pode querer permanecer como parte da sua identidade no Mirror. Quer integrar isso agora?` and routes affirmative responses to the review surface without proposing diffs or mutating identity.
