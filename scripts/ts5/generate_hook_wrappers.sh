...
# `read -d ''` from a plain heredoc, NOT `$(cat <<'EOF' ... )`: inside a command
# substitution, bash 3.2 -- macOS's /bin/bash -- joins the template's
# backslash-newline continuations even in a quoted heredoc, while bash 5 keeps
# them. The wrappers then depended on which bash generated them, and CI's
# Ubuntu leg saw all fourteen drift from the ones generated on a Mac.
