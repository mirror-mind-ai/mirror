...
def test_workspace_home_surfaces_operational_sections(
    ...
    assert home.status == "Where you find your work, projects, decisions and daily tasks."
    ...
    assert metrics["open-tasks"].value == 1
    ...
    assert sections["tasks"].cards[0].metadata["journey"] == "mirror-mind"
...
def test_workspace_home_can_select_requested_active_journey(
    ...
    sections = {section.id: section for section in home.sections}
    assert sections["tasks"].cards[0].title == "Beta task"
