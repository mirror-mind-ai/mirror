...
def test_mm_build_skill_requires_marked_ariad_surfaces_to_render_verbatim():
    ...
    # CV22.DS10.TS4: the skill must not offer a SQLite fallback anymore.
    assert "There is no second store" in skill
    assert "Legacy SQLite path" not in skill
    assert "build refinement-story create" not in skill
